...
export class OnboardingService {
  ...
  private async extractTags(
    ...
    const defaultPrompt = [
      ...
      '公开标签应该适合用在匿名匹配卡片上，描述个人的性格特点。',
      '隐藏特征应涵盖安全性、情绪稳定性、共情能力、边界感、冲突处理方式、骚扰倾向等。',
      ...
      '- 4到8个公开标签，最少4个',
      '- 5到10个隐藏特征',
      '- 绝对极度重要事项：所有的 tag_name 值（包括 public_tags 和 hidden_system_traits）绝对必须100%使用中文，禁止生成任何英文键值或英文单词。例如，必须生成“骚扰倾向”而不是 harassment_tendency，“情绪稳定”而不是 emotional_stability。',
      '- 公开标签应该是有情感含义的内容，不要仅仅是普通的兴趣爱好',
      '- onboarding_summary 必须是一句短中文总结（尽量在 50 个中文字以内）',
      '- onboarding_summary 和所有的 ai_justification 必须是中文，严禁使用英文',
    ].join('\n');
    const promptTemplate = await this.promptTemplateService.getPrompt(
