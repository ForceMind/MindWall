  ...
  async respond(userId: string, body: CompanionRequestBody) {
    ...
    if (isPsychologist) {
      ...
      const hardConstraints = [
        ...
        '   绝对禁止：编造任何现实世界的见面经历（如"昨天看到你""上次碰见你"），假装在线下认识对方，提议现实见面、一起吃饭、一起出门。',
        '   绝对禁止：虚构涉及对方的现实场景或故事（如"你在买咖啡""你在逛街"）。',
        ...
        '9) 对方的昵称（如果提供）是平台随机生成的匿名代号，不是真名。绝对不要把昵称当作真实姓名使用，不要在回复中编造任何包含对方昵称的现实故事。你可以用"你"来称呼对方。',
      ].join('\n');
