...
let OnboardingService = OnboardingService_1 = class OnboardingService {
    ...
    async saveBasicsForUser(userId, body) {
        ...
        return {
            status: 'ok',
            message: '基础资料已保存。系统会根据你的资料和后续回答，逐步修正匿名昵称、头像和匹配画像。',
            profile,
        };
    }
    async saveCityForUser(userId, body) {
    ...
    async startSession(body) {
        const authProviderId = body.auth_provider_id?.trim() || `dev_${(0, crypto_1.randomUUID)().slice(0, 12)}`;
    ...
    async startSessionForUser(userId, body) {
    ...
    async submitMessageForUser(sessionId, body, userId) {
    ...
    async submitMessage(sessionId, body) {
        const session = this.sessions.get(sessionId);
        if (!session) {
            throw new common_1.NotFoundException('Onboarding session not found.');
        }
    ...
    async submitMessageInternal(sessionId, session, body) {
    ...
    async generateQuestion(turns, turnIndex, userId) {
        ...
        const fallback = this.pickFallbackQuestion(turnIndex, previousQuestions, latestUserAnswer, userId);
        ...
        const promptTemplate = await this.promptTemplateService.getPrompt('onboarding.question', defaultPrompt);
        ...
        const data = await this.callOpenAiJson(prompt, {
            userId,
    ...
    async extractTags(turns, userId) {
        const aiConfig = await this.adminConfigService.getAiConfig();
        const apiKey = aiConfig.openaiApiKey;
        const transcript = this.renderTranscript(turns);
        if (!apiKey) {
        ...
        const promptTemplate = await this.promptTemplateService.getPrompt('onboarding.tag_extraction', defaultPrompt);
        const prompt = [
            promptTemplate,
            'Interview transcript:',
            transcript,
    ...
    async persistTags(userId, extracted) {
        const publicTags = extracted.public_tags.map((tag) => this.normalizeTag(tag, 1, '自我觉察者', '基于访谈内容生成的公开画像标签。'));
        const hiddenTags = extracted.hidden_system_traits.map((tag) => this.normalizeTag(tag, 10, 'emotional_stability', '基于访谈内容生成的隐藏系统画像。'));
    ...
    normalizeExtraction(input, turns) {
        const fallback = this.fallbackTagExtraction(turns);
        const publicTags = (input.public_tags || [])
            .map((tag) => this.normalizeTag(tag, 1, '自我觉察者', '基于访谈内容生成的公开画像标签。'))
            .slice(0, 8);
        const hiddenTags = (input.hidden_system_traits || [])
            .map((tag) => this.normalizeTag(tag, 10, 'emotional_stability', '基于访谈内容生成的隐藏系统画像。'))
            .slice(0, 10);
        ...
        return {
            ...
            hidden_system_traits: hiddenTags.length > 0 ? hiddenTags : fallback.hidden_system_traits,
            onboarding_summary: input.onboarding_summary?.trim() || fallback.onboarding_summary,
        };
    }
    normalizeTag(tag, maxWeight, fallbackName, fallbackJustification) {
        ...
        return {
            ...
            weight,
            ai_justification: (tag.ai_justification || fallbackJustification).trim().slice(0, 280),
        };
    }
    async refreshAnonymousIdentity(userId, publicTags) {
    ...
    getPreviousAssistantQuestions(turns) {
        return turns
            .filter((turn) => turn.role === 'assistant')
            .map((turn) => turn.content.trim())
            .filter((item) => Boolean(item));
    }
    getLatestUserAnswer(turns) {
        const latest = [...turns]
            .reverse()
            .find((turn) => turn.role === 'user');
        return latest?.content?.trim() || '';
    }
    pickFallbackQuestion(turnIndex, previousQuestions, latestUserAnswer, userSeedText) {
        const history = new Set(previousQuestions.map((item) => this.normalizeQuestionText(item)));
        const adaptive = this.buildAdaptiveFallbackQuestion(turnIndex, latestUserAnswer, previousQuestions);
        if (adaptive) {
            return adaptive;
        }
        ...
        for (let offset = 0; offset < total; offset += 1) {
            const candidate = this.fallbackQuestions[(baseOffset + turnIndex + offset) % total];
    ...
    getInterviewFocus(turnIndex) {
        return (this.interviewFocuses[turnIndex % this.interviewFocuses.length] ||
            '内在冲突与关系边界');
    }
    buildAdaptiveFallbackQuestion(turnIndex, latestUserAnswer, previousQuestions) {
    ...
    isRepeatedQuestion(candidate, previousQuestions) {
    ...
    normalizeGender(gender) {
        ...
        if (!normalized) {
            throw new common_1.BadRequestException('gender is required.');
        }
        const mapping = {
            male: 'male',
            man: 'male',
    ...
    normalizeAge(age) {
        ...
        if (!Number.isFinite(numeric)) {
            throw new common_1.BadRequestException('age is required.');
        }
        const normalized = Math.round(numeric);
        if (normalized < 18 || normalized > 99) {
            throw new common_1.BadRequestException('age must be between 18 and 99.');
        }
        return normalized;
    }
    normalizeCity(city) {
    ...
    buildAnonymousIdentity(seedText) {
        const seed = this.hashSeed(seedText);
        const prefix = this.anonymousPrefix[seed % this.anonymousPrefix.length] || '微澜';
        const suffix = this.anonymousSuffix[(seed >>> 3) % this.anonymousSuffix.length] ||
            '旅人';
    ...
    async attachEmbedding(tagId, sourceText, userId) {
        ...
        await this.prisma.$executeRawUnsafe(`UPDATE "user_tags" SET "embedding" = '${vectorLiteral}'::vector WHERE "id" = '${tagId}'::uuid`);
    }
    ...
    async callOpenAiJson(prompt, options) {
        ...
        try {
            const response = await fetch(`${aiConfig.openaiBaseUrl}/chat/completions`, {
                ...
                body: JSON.stringify({
                    ...
                    messages: [
                        {
                            role: 'system',
                            content: 'You are a strict JSON generator. Output only one JSON object and nothing else.',
                        },
                        {
                            role: 'user',
                            content: prompt,
            ...
            this.logger.warn(`OpenAI call error: ${error.message}`);
            await this.serverLogService.warn('onboarding.openai.error', 'openai call failed', {
                feature: options.feature,
                error: error.message,
            });
            return null;
