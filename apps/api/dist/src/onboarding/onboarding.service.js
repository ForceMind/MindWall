...
let OnboardingService = OnboardingService_1 = class OnboardingService {
    ...
    async submitMessageInternal(sessionId, session, body) {
        ...
        if (session.answerCount < session.totalQuestions) {
            const nextQuestion = await this.generateQuestion(session.turns, session.answerCount);
            session.turns.push({
                role: 'assistant',
    ...
    async generateQuestion(turns, turnIndex) {
    ...
    async persistTags(userId, extracted) {
        const publicTags = extracted.public_tags.map((tag) => this.normalizeTag(tag, 1, '真诚表达者', '基于访谈内容的公开画像标签。'));
        const hiddenTags = extracted.hidden_system_traits.map((tag) => this.normalizeTag(tag, 10, 'emotional_stability', '基于访谈内容的系统内部画像。'));
    ...
    normalizeExtraction(input, turns) {
        ...
        const publicTags = (input.public_tags || [])
            .map((tag) => this.normalizeTag(tag, 1, '真诚表达者', '基于访谈内容的公开画像标签。'))
            .slice(0, 8);
        const hiddenTags = (input.hidden_system_traits || [])
            .map((tag) => this.normalizeTag(tag, 10, 'emotional_stability', '基于访谈内容的系统内部画像。'))
            .slice(0, 10);
    ...
    async attachEmbedding(tagId, sourceText) {
