...
let ContactsService = class ContactsService {
    ...
    async getCandidateContacts(userId) {
        const me = await this.prisma.user.findUnique({
            ...
            select: {
                ...
                tags: {
                    where: { type: client_1.UserTagType.PUBLIC_VISIBLE },
                    select: {
                        tag_name: true,
                        weight: true,
                    },
        ...
        const matchByCounterpart = new Map(matches.map((item) => [
        ...
        const candidates = users
            ...
            const publicTags = item.tags
                .filter((tag) => tag.type === client_1.UserTagType.PUBLIC_VISIBLE)
            ...
            const harassmentScore = item.tags.find((tag) => tag.type === client_1.UserTagType.HIDDEN_SYSTEM &&
                tag.tag_name === 'harassment_tendency')?.weight || 1;
    ...
    async getConnectedContacts(userId) {
        const matches = await this.prisma.match.findMany({
            where: {
        ...
        const profileMap = new Map(profiles.map((item) => [
            item.user_id,
            {
    ...
    async connectToUser(userId, targetUserId) {
        if (!targetUserId || targetUserId === userId) {
            throw new common_1.NotFoundException('Target user is invalid.');
        }
    ...
    computeCandidateScore(selfTags, otherTags, harassmentScore) {
